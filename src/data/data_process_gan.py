...
from src.data.data_process import DataProcess, print_error
...
class GANDataProcessTestPhase(DataProcess):
    ...
    def __init__(self, data_queue, data_dict, repeat=False):
        ...
        problematic_nrrd_path = os.path.join(
            cfg.PROCESSED_DATA_DIR, 'problematic_nrrds_shapenet_unverified_256_filtered_div_with_err_textures.p')
    ...
    def run(self):
        ...
        while not self.exit.is_set() and self.cur < self.num_data:
            ...
            learned_embedding_batch = np.array(
                learned_embedding_list).astype(np.float32)
        ...
        log.info('Exiting enqueue process')
    ...
    def __init__(self, data_queue, data_dict, repeat):
        ...
        Args:
            ...
            data_dict: A dict with keys 'caption_tuples' and 'caption_matches'. caption_tuples is
                ...
                is a list of the indices (ints) of caption tuples that describe the same model ID.
        """
        super(GANDataProcess, self).__init__(
            data_queue, data_dict, repeat=repeat)
    ...
    def shuffle_db_inds(self):
        ...
        if self.repeat:
            self.perms = [np.random.permutation(
                np.arange(self.num_data)) for _ in range(4)]
        else:
            # self.perms = [np.random.permutation(np.arange(self.num_data)) for _ in range(4)]
            log.info('----------------------------------------- not shuffling data')
            self.perms = [np.arange(self.num_data)
                          for _ in range(4)]  # Dont shuffle
        self.cur = 0
    ...
    def __init__(self, data_queue, data_dict, repeat):
        ...
        Args:
            ...
            data_dict: A dict with keys 'caption_tuples' and 'caption_matches'. caption_tuples is
                ...
                is a list of the indices (ints) of caption tuples that describe the same model ID.
        """
        super(CWGANMetricEmbeddingDataProcess, self).__init__(
            data_queue, data_dict, repeat=repeat)
